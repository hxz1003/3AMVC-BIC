clear;  % 清空当前 MATLAB 工作区变量，避免旧变量影响本次 Reuters-1200 粗网格搜索。
...
config.betaList = [1 10 1000];  % 3AMVC 主优化中的 beta 候选；控制锚图 Z 的正则强度，越大通常越抑制 Z 的幅度。
config.lambdaList = [1e2 1e3 1e4];  % 3AMVC 对齐阶段的 lambda 候选；Reuters-1200 使用较小范围以适配该数据尺度。
config.lambdaBICList = [0.5 1 0.22];  % BIC-LR 中 BIC 惩罚系数候选；越大越保守，通常生成更少锚点。
config.minNodeSizeList = [8 16 22];  % BIC-LR 子节点最小样本数候选；Reuters-1200 类别/视图规模较小，因此下限从 8 开始。
...
results = run_biclr_grid_search('Reuters-1200', config);  % 运行 Reuters-1200 的 BIC-LR+3AMVC 网格搜索并保存结果。
