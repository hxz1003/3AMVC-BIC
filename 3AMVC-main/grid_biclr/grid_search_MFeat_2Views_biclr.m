clear;  % 清空当前 MATLAB 工作区变量，避免旧变量影响本次 MFeat_2Views 粗网格搜索。
...
config.betaList = [80 100 160 240];  % MFeat 已有结果显示 beta=100 最稳，且新对齐下 beta 越大表现越好，因此向上扩展。
config.lambdaList = [1e3 1e4 3e4 1e5];  % 新对齐下最优出现在 1e4，保留 1e3/1e5 并补充 3e4 检查中高结构权重。
config.lambdaBICList = [0.5 1 2 3];  % 新对齐粗搜中 lambdaBIC=2 最优，同时保留旧结果稳定的 0.5/1 并向 3 延伸。
config.minNodeSizeList = [20 30 40 50];  % 新对齐最优在 20，均值趋势偏向 40；围绕两者细化并测试更保守的 50。
...
config.useCache = false;  % 是否启用锚点缓存；true 会复用/保存 cacheDir 中同参数的单视图 BIC-LR 锚点结果。
...
results = run_biclr_grid_search('MFeat_2Views', config);  % 运行 MFeat_2Views 的 BIC-LR+3AMVC 网格搜索并保存结果。
