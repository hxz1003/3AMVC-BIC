...
for iv = 1:v
    ...
    infoAll{iv} = info;
    fprintf('视图 %d：锚点数=%d，SSE=%.6f，耗时=%.2fs\n', iv, class, object_sum(iv), time_neighbor);
end
...
[resultMean, resultStd] = myNMIACCwithmean(U, Y, k, evalOptions);
totalTime = toc(totalTimer);

fprintf('algo_qp 迭代次数=%d，最终目标值=%.6f\n', iter, obj(end));
fprintf('每个视图锚点数=%s\n', mat2str(cellfun(@(s) s.numAnchors, infoAll)'));
fprintf('ACC=%.4f±%.4f，NMI=%.4f±%.4f，Purity=%.4f±%.4f，Fscore=%.4f±%.4f\n', ...
