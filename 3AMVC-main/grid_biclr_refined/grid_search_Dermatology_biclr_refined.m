clear;  % 清空当前 MATLAB 工作区变量，避免旧变量影响本次 Dermatology 精细搜索。
...
config.betaList = [50 100 150 200 300];  % 200335 结果中 bestMean 落在 beta=100 的粗搜上界，向更强正则方向外扩。
config.lambdaList = [1e2 3e2 1e3 3e3 1e4];  % 最优重复均值位于 lambda=1000 的粗搜上界，向 3000/10000 继续探测。
config.lambdaBICList = [1 1.5 2 2.5 3];  % bestSummary/bestMean 均在 lambdaBIC=2 的粗搜上界，向更保守分裂方向细化。
config.minNodeSizeList = [10 16 20 24 32];  % bestSummary/bestMean 均在 minNodeSize=20 的粗搜上界，保留 10 并向 32 外扩。
...
config.removeClutter = false;  % Dermatology 不涉及 clutter 类，保留该字段以保持脚本字段风格统一。
...
results = run_biclr_grid_search('Dermatology', config);  % 运行 Dermatology 的 BIC-LR+3AMVC 精细搜索并保存结果。
...
fprintf('Dermatology 精细搜索完成。固定随机种子=%d。\n', seed);
fprintf(['粗搜参考 bestMean/bestUpper：beta=100，lambda=1000，lambdaBIC=2，minNodeSize=20，' ...
    'ACC=0.8375±0.0552，NMI=0.7652，summaryACC=0.9246。\n']);
fprintf(['粗搜参考 bestSummary：beta=10，lambda=1000，lambdaBIC=2，minNodeSize=20，' ...
    'summaryACC=0.9302，ACC=0.7910±0.0758。\n']);
