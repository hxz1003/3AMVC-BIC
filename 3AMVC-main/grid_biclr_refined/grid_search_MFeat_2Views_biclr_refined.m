clear;  % 清空当前 MATLAB 工作区变量，避免旧变量影响本次 MFeat_2Views 精细搜索。
...
config.betaList = [60 80 100 120 160];  % 191006 结果中 beta=80/100/160 出现高值，向 beta=80 的低边界外扩到 60。
config.lambdaList = [1e3 1e4 3e4 1e5];  % 191006 结果的高值主要落在 1e3、1e4、3e4、1e5，保留这些对齐强度候选。
config.lambdaBICList = [1.5 2 2.5 3 3.5];  % 重复均值更优区域集中在 lambdaBIC=2/3，并向 3 的高边界外扩到 3.5。
config.minNodeSizeList = [20 30 40 50 60];  % 稳定高值集中在 30/50，同时保留单次高点 20，并向 50 的高边界外扩到 60。
...
config.evalOptions = struct( ...  % 聚类评价阶段参数，控制 myNMIACCwithmean 中 KMeans 重复评价方式。
    'numRuns', 10, ...  % 外层评价重复次数；精细搜索比粗搜索更重视结果稳定性。
...
config.removeClutter = false;  % MFeat_2Views 不涉及 clutter 类，保留该字段以保持脚本字段风格统一。
...
results = run_biclr_grid_search('MFeat_2Views', config);  % 运行 MFeat_2Views 的 BIC-LR+3AMVC 精细搜索并保存结果。
...
fprintf('MFeat_2Views 精细搜索完成。固定随机种子=%d。\n', seed);
