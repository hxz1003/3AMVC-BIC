%% 
clear;  % 清空当前 MATLAB 工作区变量，避免旧变量影响本次 Reuters-1200 精细搜索。
...
config.betaList = [500 800 1000 1500 2000];  % 193824 结果中 beta=1000 明显优于 1/10，围绕 1000 加密并向高侧扩展。
config.lambdaList = [1e2 1e3 1e4 3e4];  % 最优点位于 lambda=1e4 的粗搜上界，保留 1e2/1e3 并向 3e4 外扩。
config.lambdaBICList = [0.1 0.15 0.22 0.35 0.5];  % 最优点位于 lambdaBIC=0.22 的低边界，向更弱惩罚 0.1/0.15 细化。
config.minNodeSizeList = [8 12 16 22 28 32];  % 最优点位于 minNodeSize=22 的粗搜上界，同时保留 minNodeSize=8 的第二高点。
config.anchorOptions = struct( ...  % BIC-LR 锚点生成的固定工程参数，和 lambdaBIC/minNodeSize 网格共同决定锚点。
    ...
    'maxAnchors', 300, ...  % 单视图最大锚点数安全上限；Reuters-1200 设置为 300 以限制计算量。
...
config.evalOptions = struct( ...  % 聚类评价阶段参数，控制 myNMIACCwithmean 中 KMeans 重复评价方式。
    'numRuns', 8, ...  % 外层评价重复次数；Reuters-1200 粗搜索标准差较大，精细搜索增加重复次数。
...
config.removeClutter = false;  % Reuters-1200 不涉及 clutter 类，保留该字段以保持脚本字段风格统一。
...
results = run_biclr_grid_search('Reuters-1200', config);  % 运行 Reuters-1200 的 BIC-LR+3AMVC 精细搜索并保存结果。
...
fprintf('Reuters-1200 精细搜索完成。固定随机种子=%d。\n', seed);
