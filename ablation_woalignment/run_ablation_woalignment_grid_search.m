function results = run_ablation_woalignment_grid_search(datasetName, config)
%RUN_ABLATION_WOALIGNMENT_GRID_SEARCH 运行 w/o alignment 粗网格搜索。
%   本函数保留完整 BIC-LR 锚点生成和基准视图选择，但跳过跨视图对齐与融合，
%   仅使用基准视图进入最终谱嵌入与 KMeans，按 best ACC 选优。
...
totalComb = numel(config.betaList) * numel(config.lambdaBICList) * numel(config.minNodeSizeList);
...
for ibic = 1:numel(config.lambdaBICList)
    ...
    for imin = 1:numel(config.minNodeSizeList)
        ...
        for ibeta = 1:numel(config.betaList)
            ...
            fprintf(['[w/o alignment][%d/%d] %s | beta=%g | lambdaBIC=%g | minNodeSize=%d | ' ...
                'anchors=%s | targetView=%d | ACC=%.4f | NMI=%.4f | ' ...
                'Purity=%.4f | Fscore=%.4f | algoTime=%.2fs | anchorTime=%.2fs\n'], ...
                recordId, totalComb, datasetName, beta, lambdaBIC, minNodeSize, ...
...
results = struct();
results.methodName = 'woalignment';
...
savePath = fullfile(config.saveDir, sprintf('%s_woalignment_grid_%s.mat', sanitize_key(datasetName), timestamp));
results.savePath = savePath;
save(savePath, 'results', '-v7.3');

fprintf('===== w/o alignment 粗网格搜索结束 =====\n');
fprintf(['按 %s 最高选择最优结果：beta=%g，lambdaBIC=%g，minNodeSize=%d，' ...
    'ACC=%.4f，NMI=%.4f，Purity=%.4f，Fscore=%.4f，总耗时=%.2fs\n'], ...
    config.selectionMetricName, results.best.beta, results.best.lambdaBIC, ...
...
switch lower(datasetName)
    ...
    case 'wikifea'
        ...
        grid.kmeansReplicates = 2;
    otherwise
        error('run_ablation_woalignment_grid_search:UnsupportedDataset', ...
...
for iv = 1:numViews
    cacheKey = sprintf('%s_%s_view%d_woalignment_lamBIC%s_minNode%d_seed%d', ...
...
if isempty(primaryIdx)
    error('run_ablation_woalignment_grid_search:UnknownMetric', '未找到指标 %s。', selectionMetricName);
end
