function results = run_ablation_wolr_grid_search(datasetName, config)
%RUN_ABLATION_WOLR_GRID_SEARCH 运行 w/o LR 粗网格搜索。
%   本函数对指定数据集执行“去掉似然比，仅保留 DeltaSSE 二分准则”的粗筛实验，
%   并按 best ACC 选出最优参数组合。
...
for imin = 1:numel(config.minNodeSizeList)
    ...
    for ibeta = 1:numel(config.betaList)
        ...
        for ilambda = 1:numel(config.lambdaList)
            ...
            fprintf(['[w/o LR][%d/%d] %s | beta=%g | lambda=%g | minNodeSize=%d | ' ...
...
savePath = fullfile(config.saveDir, sprintf('%s_wolr_grid_%s.mat', sanitize_key(datasetName), timestamp));
results.savePath = savePath;
save(savePath, 'results', '-v7.3');

fprintf('===== w/o LR 粗网格搜索结束 =====\n');
...
switch lower(datasetName)
    ...
    case 'wikifea'
        ...
        grid.kmeansReplicates = 2;
    otherwise
        error('run_ablation_wolr_grid_search:UnsupportedDataset', ...
...
for iv = 1:numViews
    cacheKey = sprintf('%s_%s_view%d_wolr_minNode%d_seed%d', ...
    ...
    anchorTimer = tic;
    [~, ~, label_neighbor, object, theta, ~, info] = Neighbor_BICLR_woLR(X{iv}, Y, anchorOptions);
...
if isempty(primaryIdx)
    error('run_ablation_wolr_grid_search:UnknownMetric', '未找到指标 %s。', selectionMetricName);
end
