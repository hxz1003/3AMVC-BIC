# 3. Proposed Method

## 3.1 问题定义

给定一个包含 $V$ 个视图的多视图数据集：

$$\mathcal{X}=\{X^{(v)}\}_{v=1}^{V},$$

其中第 $v$ 个视图表示为：

$$X^{(v)}  
=  
\left[  
x_1^{(v)},x_2^{(v)},\ldots,x_n^{(v)}  
\right]^\top  
\in \mathbb{R}^{n\times d_v}.$$

这里，$n$ 表示样本数，$d_v$ 表示第 $v$ 个视图的特征维度，$x_i^{(v)}\in\mathbb{R}^{d_v}$ 表示第 $i$ 个样本在第 $v$ 个视图下的特征表示。多视图聚类的目标是在没有监督标签的情况下，学习一个统一的聚类划分：

$$\mathcal{Y}=\{y_i\}_{i=1}^{n},  
\qquad  
y_i\in\{1,2,\ldots,K\},$$

其中 $K$ 表示聚类类别数。

锚图多视图聚类的基本思想是：不直接构造完整的 $n\times n$ 样本相似图，而是在每个视图中选取较少数量的代表性锚点，并通过样本—锚点关系近似原始样本图结构。3AMVC 论文指出，锚图方法能够有效降低完整图构造中的二次复杂度，使多视图聚类能够适应大规模数据场景。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

对于第 $v$ 个视图，本文需要自动学习一组锚点：

$$\Theta^{(v)}  
=  
\left\{  
\theta_1^{(v)},\theta_2^{(v)},\ldots,\theta_{m_v}^{(v)}  
\right\},$$

其中 $m_v$ 表示第 $v$ 个视图自动生成的锚点数。与预先固定所有视图锚点数的方法不同，本文允许不同视图具有不同数量的锚点，即：

$$m_1\neq m_2\neq\cdots\neq m_V.$$

这与 3AMVC 的基本动机一致。3AMVC 认为，强制不同视图具有相同数量的锚点会限制各视图自身的表示能力，因此需要一种能够为每个视图自适应确定锚点数量的机制。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

---

## 3.2 总体框架

本文方法沿用 3AMVC 的整体框架，即：

$$X^{(v)}  
\longrightarrow  
\Theta^{(v)}  
\longrightarrow  
Z^{(v)}  
\longrightarrow  
\widetilde Z^{(v)}  
\longrightarrow  
Z_{\mathrm{aligned}}  
\longrightarrow  
\mathcal{Y}.$$

整个方法由五个主要阶段组成。

第一，对每个视图独立执行 **基于 BIC 正则化似然比检验的自适应锚点选择**，得到视图特定的锚点集合 $\Theta^{(v)}$。

第二，基于锚点集合构造每个视图的样本—锚点关系矩阵 $Z^{(v)}$。

第三，根据单视图锚点质量选择基准视图 $b$。

第四，将其他视图的锚图对齐到基准视图，得到对齐后的锚图 $\widetilde Z^{(v)}$。

第五，对所有对齐后的锚图执行等权融合，得到统一的融合锚图 $Z_{\mathrm{aligned}}$，再通过谱嵌入与聚类得到最终结果。

3AMVC 论文中的总体框架同样包括三个核心步骤：首先在各视图中自动选择代表性锚点，其次根据锚图质量选择基准视图并对齐其他视图，最后融合对齐后的锚图并进行聚类。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

### 3.2.1 原 3AMVC 的对齐与基准视图选择机制

原始 3AMVC 的关键设定是：每个视图先独立生成锚点和锚图，而不是预先强制所有视图共享同一组锚点。由于不同视图的特征维度、局部密度和可分结构不同，第 $v$ 个视图自动得到的锚点数 $m_v$ 可以与其他视图不同。该自适应机制提高了单视图表示能力，但也引入了锚点不对齐问题：当 $m_v\neq m_u$ 时，两个视图的锚图分别属于不同的锚点坐标系，其列索引不再具有一一对应关系，不能直接相加或求平均。

为解决这一问题，3AMVC 先根据单视图锚点质量选择一个基准视图 $b$。原方法主要利用锚点划分后的簇内误差或代表性质量作为评价依据，误差越小表示该视图的锚点越能紧凑地代表样本分布。选定基准视图后，其他视图的锚图通过对齐矩阵映射到基准视图的锚点空间，从而使不同视图的样本—锚点关系具有统一维度。

对齐矩阵的作用不是重新学习样本标签，而是在锚点层面建立跨视图对应关系。对于非基准视图 $v\neq b$，对齐矩阵 $P_v$ 将其锚点空间映射到基准锚点空间，使 $Z^{(v)}P_v$ 与 $Z^{(b)}$ 具有相同列维度。这样，即使不同视图的锚点数不同，也可以在统一锚点空间中融合多视图信息。

完成对齐后，原 3AMVC 采用等权融合，即每个已对齐视图以相同权重参与最终共识锚图构造。等权融合与加权融合的区别在于：前者不为不同视图额外学习或指定融合权重，后者则需要引入视图权重变量或权重超参数。本文当前主线采用原 3AMVC 的等权融合，仅改进锚点选择与基准视图选择准则，不引入质量加权融合、对齐残差加权融合或新的融合超参数。

---

# 3.3 基于 BIC 正则化似然比检验的自适应锚点选择

## 3.3.1 局部节点建模

对于第 $v$ 个视图中的任意当前节点，记其样本集合为：

$$C=\{x_i^{(v)}\mid i\in\mathcal{I}_C\}.$$

其中，$\mathcal{I}_C$ 表示属于当前节点的样本索引集合，节点样本数为：

$$n_C=|C|.$$

为简化记号，在本节中省略视图上标 $(v)$，将样本记为：

$$x_i\in\mathbb{R}^{d}.$$

当前节点 $C$ 的均值中心定义为：

$$\mu_C  
=  
\frac{1}{n_C}  
\sum_{x_i\in C}x_i.  
\tag{1}$$

其簇内平方误差定义为：

$$W(C)  
=  
\sum_{x_i\in C}  
\left\|  
x_i-\mu_C  
\right\|_2^2.  
\tag{2}$$

该项用于衡量当前节点内部样本围绕中心的紧凑程度。3AMVC 原文在讨论 HBNC 停止准则时，也将样本到簇中心的距离作为评价簇中心代表性的重要依据：簇内样本越接近其中心，簇中心作为锚点的代表性越强。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

---

## 3.3.2 单簇假设、双簇假设与似然估计

对于当前节点 $C$，本文将“是否继续划分”转化为一个局部统计模型选择问题。具体地，考虑如下两个竞争假设：

$$H_0:\ C\ \text{由一个球形高斯分布生成},$$ $$H_1:\ C\ \text{由两个共享方差的球形高斯子分布生成}.$$

其中，$H_0$ 表示当前节点内部已经足够紧凑，不需要继续生成新的锚点；$H_1$ 表示当前节点内部仍然存在可分结构，应当继续二分，从而产生更多局部锚点。

---

### 3.3.2.1 零假设下的单簇似然函数

在零假设 $H_0$ 下，假设当前节点 $C$ 中所有样本均由同一个球形高斯分布生成：

$$x_i\sim \mathcal{N}(\mu_0,\sigma_0^2 I_d),  
\qquad  
x_i\in C.  
\tag{3}$$

其中，$\mu_0\in\mathbb{R}^{d}$ 为均值向量，$\sigma_0^2$ 为球形方差，$I_d$ 为 $d$ 维单位矩阵。

对于单个样本 $x_i$，其概率密度为：

$$p(x_i\mid \mu_0,\sigma_0^2)  
=  
\frac{1}{(2\pi\sigma_0^2)^{d/2}}  
\exp  
\left(  
-\frac{  
\|x_i-\mu_0\|_2^2  
}{  
2\sigma_0^2  
}  
\right).  
\tag{4}$$

由于节点内样本被假设为独立同分布，因此当前节点在 $H_0$ 下的似然函数为：

$$L_0(C\mid \mu_0,\sigma_0^2)  
=  
\prod_{x_i\in C}  
p(x_i\mid \mu_0,\sigma_0^2).  
\tag{5}$$

将式 $(4)$ 代入式 $(5)$，得到：

$$L_0(C\mid \mu_0,\sigma_0^2)  
=  
\prod_{x_i\in C}  
\frac{1}{(2\pi\sigma_0^2)^{d/2}}  
\exp  
\left(  
-\frac{  
\|x_i-\mu_0\|_2^2  
}{  
2\sigma_0^2  
}  
\right).  
\tag{6}$$

对式 $(6)$ 取对数，可得零假设下的对数似然函数：

$$\ell_0(C\mid \mu_0,\sigma_0^2)  
=  
-\frac{n_Cd}{2}\log(2\pi)  
-  
\frac{n_Cd}{2}\log\sigma_0^2  
-  
\frac{1}{2\sigma_0^2}  
\sum_{x_i\in C}  
\|x_i-\mu_0\|_2^2.  
\tag{7}$$

为了获得最大似然估计，对 $\mu_0$ 求导并令其为零，可得到：

$$\hat{\mu}_0  
=  
\frac{1}{n_C}  
\sum_{x_i\in C}x_i  
=  
\mu_C.  
\tag{8}$$

再对 $\sigma_0^2$ 求导并令其为零，可得到：

$$\hat{\sigma}_0^2  
=  
\frac{1}{n_Cd}  
\sum_{x_i\in C}  
\left\|  
x_i-\hat{\mu}_0  
\right\|_2^2.  
\tag{9}$$

结合式 $(2)$，式 $(9)$ 可写为：

$$\hat{\sigma}_0^2  
=  
\frac{W(C)}{n_Cd}.  
\tag{10}$$

为避免节点内部样本完全重合时出现方差退化，引入一个极小正数 $\varepsilon>0$，得到稳定形式：

$$\hat{\sigma}_0^2  
=  
\frac{W(C)}{n_Cd}  
+  
\varepsilon.  
\tag{11}$$

将式 $(8)$ 和式 $(11)$ 代入式 $(7)$，可得单簇模型的极大对数似然：

$$\ell_0(C)  
=  
-\frac{n_Cd}{2}  
\left[  
\log(2\pi\hat{\sigma}_0^2)+1  
\right].  
\tag{12}$$

式 $(3)$–$(12)$ 给出了单簇假设下的完整似然估计过程。其含义是：若当前节点中的样本能够被一个均值中心和一个共享球形方差较好解释，则继续划分该节点的必要性较弱。

---

### 3.3.2.2 备择假设下的双簇似然函数

在备择假设 $H_1$ 下，当前节点 $C$ 被划分为两个子节点：

$$C\rightarrow C_1\cup C_2,  
\qquad  
C_1\cap C_2=\varnothing.  
\tag{13}$$

记两个子节点的样本数分别为：

$$n_1=|C_1|,  
\qquad  
n_2=|C_2|,  
\qquad  
n_1+n_2=n_C.  
\tag{14}$$

双簇模型假设两个子簇分别服从不同均值但共享方差的球形高斯分布：

$$x_i\mid x_i\in C_g  
\sim  
\mathcal{N}(\mu_g,\sigma_{12}^2 I_d),  
\qquad  
g=1,2.  
\tag{15}$$

其中，$\mu_1,\mu_2\in\mathbb{R}^{d}$ 分别表示两个子簇的均值向量，$\sigma_{12}^2$ 表示两个子簇共享的球形方差。

两个子簇的混合比例定义为：

$$\pi_1=\frac{n_1}{n_C},  
\qquad  
\pi_2=\frac{n_2}{n_C},  
\qquad  
\pi_1+\pi_2=1.  
\tag{16}$$

若给定当前候选划分 $C_1,C_2$，则双簇模型的似然函数可写为：

$$L_1(C_1,C_2\mid \mu_1,\mu_2,\sigma_{12}^2,\pi_1,\pi_2)  
=  
\prod_{g=1}^{2}  
\prod_{x_i\in C_g}  
\pi_g  
\frac{1}{(2\pi\sigma_{12}^2)^{d/2}}  
\exp  
\left(  
-\frac{  
\|x_i-\mu_g\|_2^2  
}{  
2\sigma_{12}^2  
}  
\right).  
\tag{17}$$

对式 $(17)$ 取对数，可得备择假设下的对数似然函数：

$$\ell_1(C_1,C_2)  
=  
\sum_{g=1}^{2}  
\sum_{x_i\in C_g}  
\log \pi_g  
-  
\frac{n_Cd}{2}\log(2\pi)  
-  
\frac{n_Cd}{2}\log\sigma_{12}^2  
-  
\frac{1}{2\sigma_{12}^2}  
\sum_{g=1}^{2}  
\sum_{x_i\in C_g}  
\|x_i-\mu_g\|_2^2.  
\tag{18}$$

由于：

$$\sum_{x_i\in C_g}\log\pi_g  
=  
n_g\log\pi_g,$$

式 $(18)$ 可进一步写为：

$$\ell_1(C_1,C_2)  
=  
n_1\log\pi_1+n_2\log\pi_2  
-  
\frac{n_Cd}{2}\log(2\pi)  
-  
\frac{n_Cd}{2}\log\sigma_{12}^2  
-  
\frac{1}{2\sigma_{12}^2}  
\sum_{g=1}^{2}  
\sum_{x_i\in C_g}  
\|x_i-\mu_g\|_2^2.  
\tag{19}$$

对于给定划分，两个子簇均值的极大似然估计分别为：

$$\hat{\mu}_1  
=  
\frac{1}{n_1}  
\sum_{x_i\in C_1}x_i,  
\qquad  
\hat{\mu}_2  
=  
\frac{1}{n_2}  
\sum_{x_i\in C_2}x_i.  
\tag{20}$$

两个子簇的簇内平方误差分别为：

$$W(C_1)  
=  
\sum_{x_i\in C_1}  
\left\|  
x_i-\hat{\mu}_1  
\right\|_2^2,$$ $$W(C_2)  
=  
\sum_{x_i\in C_2}  
\left\|  
x_i-\hat{\mu}_2  
\right\|_2^2.  
\tag{21}$$

共享方差的极大似然估计为：

$$\hat{\sigma}_{12}^2  
=  
\frac{  
W(C_1)+W(C_2)  
}{  
n_Cd  
}.  
\tag{22}$$

同样，为保证数值稳定性，引入 $\varepsilon>0$，得到：

$$\hat{\sigma}_{12}^2  
=  
\frac{  
W(C_1)+W(C_2)  
}{  
n_Cd  
}  
+  
\varepsilon.  
\tag{23}$$

混合比例的极大似然估计为：

$$\hat{\pi}_1=\frac{n_1}{n_C},  
\qquad  
\hat{\pi}_2=\frac{n_2}{n_C}.  
\tag{24}$$

将式 $(20)$、$(23)$、$(24)$ 代入式 $(19)$，得到双簇模型的极大对数似然：

$$\ell_1(C_1,C_2)  
=  
n_1\log\hat{\pi}_1  
+  
n_2\log\hat{\pi}_2  
-  
\frac{n_Cd}{2}  
\left[  
\log(2\pi\hat{\sigma}_{12}^2)+1  
\right].  
\tag{25}$$

式 $(25)$ 包含两部分含义。第一部分：

$$n_1\log\hat{\pi}_1+n_2\log\hat{\pi}_2$$

刻画两个子簇规模比例对似然的贡献。当划分极度不均衡时，该项会降低双簇模型的似然得分，从而抑制过小子簇。第二部分：

$$-\frac{n_Cd}{2}  
\left[  
\log(2\pi\hat{\sigma}_{12}^2)+1  
\right]$$

刻画双簇模型对节点内部样本分布的拟合程度。若二分后簇内平方误差明显下降，则 $\hat{\sigma}_{12}^2$ 变小，双簇模型的对数似然将增加。

上述似然结构对应于当前改进模型中使用的数学形式：单簇模型用节点整体簇内平方误差估计方差，双簇模型用两个子节点的簇内平方误差之和估计共享方差，并通过混合比例项约束划分均衡性。[![GitHub](data:image/png;base64,)GitHub](https://raw.githubusercontent.com/hxz1003/3AMVC-/master/3AMVC-main/BIC_LR_HBNC.m)

---

### 3.3.2.3 似然比增益

对于一个候选划分 $C\rightarrow C_1\cup C_2$，定义单簇模型与双簇模型之间的似然比统计量：

$$\Lambda(C_1,C_2)  
=  
2\left[  
\ell_1(C_1,C_2)-\ell_0(C)  
\right].  
\tag{26}$$

若 $\Lambda(C_1,C_2)$ 较大，说明双簇模型相较于单簇模型显著提升了对当前节点数据的解释能力；反之，若 $\Lambda(C_1,C_2)$ 较小，则说明继续划分带来的拟合收益有限。

然而，单纯比较似然会偏向参数更多的双簇模型。因为双簇模型包含更多均值参数和混合比例参数，其拟合能力天然强于单簇模型。因此，本文进一步引入 BIC 复杂度惩罚。Schwarz 提出的 BIC 准则本质上是在极大似然的基础上加入模型维度惩罚，用于在不同参数维度的模型之间进行选择。[![](https://www.google.com/s2/favicons?domain=https://sites.stat.washington.edu&sz=128)华盛顿大学统计网站](https://sites.stat.washington.edu/courses/stat527/s13/readings/ann_stat1978.pdf?utm_source=chatgpt.com)

在单簇模型中，参数包括一个 $d$ 维均值向量和一个方差参数，因此参数量为：

$$p_0=d+1.  
\tag{27}$$

在双簇模型中，参数包括两个 $d$ 维均值向量、一个共享方差参数，以及一个独立混合比例参数，因此参数量为：

$$p_1=2d+2.  
\tag{28}$$

于是，双簇模型相对于单簇模型新增的参数量为：

$$\Delta p=p_1-p_0=d+1.  
\tag{29}$$

根据 BIC 模型选择思想，双簇模型只有在似然提升足以抵消复杂度惩罚时才应被接受。因此定义 BIC 正则化似然比得分：

$$S(C_1,C_2)  
=  
2\left[  
\ell_1(C_1,C_2)-\ell_0(C)  
\right]  
-  
\lambda_{\mathrm{BIC}}  
(d+1)\log n_C.  
\tag{30}$$

其中，$\lambda_{\mathrm{BIC}}>0$ 为 BIC 惩罚系数。当 $\lambda_{\mathrm{BIC}}=1$ 时，式 $(30)$ 对应标准 BIC 惩罚形式；当 $\lambda_{\mathrm{BIC}}>1$ 时，划分更加保守；当 $0<\lambda_{\mathrm{BIC}}<1$ 时，划分更容易被接受。

该判据与 X-means 中利用 BIC 判断局部簇是否继续分裂的思想一致：不预先固定最终簇数，而是通过信息准则决定当前局部结构是否需要进一步细分。[![](https://www.google.com/s2/favicons?domain=https://www.cs.cmu.edu&sz=128)CMU School of Computer Science+1](https://www.cs.cmu.edu/~dpelleg/download/xmeans.pdf?utm_source=chatgpt.com)

最终，如果存在候选划分满足：

$$S(C_1,C_2)>\tau_{\mathrm{split}},  
\tag{31}$$

则接受该划分；否则停止划分当前节点，并将当前节点中心作为锚点。这里，$\tau_{\mathrm{split}}$ 为划分接受阈值，通常可取 $0$。当 $\tau_{\mathrm{split}}=0$ 时，模型只有在双簇假设经过 BIC 惩罚后仍优于单簇假设时才接受划分。

需要说明的是，混合模型或分裂模型中的似然比检验常常涉及边界参数、未识别参数或非标准渐近分布，因此直接使用固定卡方阈值并不总是严格成立。Self 和 Liang 对非标准条件下最大似然估计与似然比统计量的渐近性质进行了系统讨论。因此，本文采用 BIC 正则化似然比作为局部模型选择准则，而不是直接依赖固定自由度的卡方显著性检验。[![](https://www.google.com/s2/favicons?domain=https://www.stat.cmu.edu&sz=128)卡内基梅隆大学统计与数据科学](https://www.stat.cmu.edu/~brian/763-2015/week06/papers/self-liang-1987.pdf?utm_source=chatgpt.com)

---

## 3.3.3 投影排序与候选切分点搜索

为了避免在高维空间中枚举所有可能的二分划分，本文首先在当前节点中构造一个局部分裂方向。设当前节点中得到两个粗分离中心：

$$c_1,c_2\in\mathbb{R}^{d}.$$

定义投影方向为：

$$u  
=  
\frac{c_1-c_2}  
{\|c_1-c_2\|_2+\varepsilon}.  
\tag{32}$$

然后，将当前节点中的每个样本投影到该方向上：

$$s_i=u^\top x_i,  
\qquad x_i\in C.  
\tag{33}$$

将投影值升序排列：

$$s_{(1)}\le s_{(2)}\le\cdots\le s_{(n_C)}.  
\tag{34}$$

由于投影方向刻画了当前节点中最主要的二分离趋势，因此只需要沿该一维排序序列扫描候选切分点。设每个子节点允许的最小样本数为 $n_{\min}$，则候选切分位置为：

$$k\in  
\{n_{\min},n_{\min}+1,\ldots,n_C-n_{\min}\}.  
\tag{35}$$

每个候选位置 $k$ 对应一个二分：

$$C_1(k)  
=  
\{x_{(1)},x_{(2)},\ldots,x_{(k)}\},$$ $$C_2(k)  
=  
\{x_{(k+1)},x_{(k+2)},\ldots,x_{(n_C)}\}.  
\tag{36}$$

对每个候选划分 $C_1(k),C_2(k)$，根据式 $(12)$、式 $(25)$ 和式 $(30)$ 计算 BIC 正则化似然比得分：

$$S_k  
=  
2\left[  
\ell_1(C_1(k),C_2(k))-\ell_0(C)  
\right]  
-  
\lambda_{\mathrm{BIC}}  
(d+1)\log n_C.  
\tag{37}$$

选择最优切分位置：

$$k^\ast  
=  
\arg\max_k S_k.  
\tag{38}$$

若：

$$S_{k^\ast}>\tau_{\mathrm{split}},  
\tag{39}$$

则接受划分：

$$C\rightarrow C_1(k^\ast)\cup C_2(k^\ast).  
\tag{40}$$

否则，当前节点停止划分。当前项目所表达的模型正是先构造局部分裂方向，再沿投影排序扫描候选二分点，并以 BIC 正则化似然比得分决定节点是否继续分裂。[![GitHub](data:image/png;base64,)GitHub](https://raw.githubusercontent.com/hxz1003/3AMVC-/master/3AMVC-main/BIC_LR_HBNC.m)

需要强调的是，投影排序只用于限制候选切分点的搜索空间；似然估计中的簇内平方误差仍然基于原始 $d$ 维样本计算。因此，该过程并不是简单的一维聚类，而是利用一维投影提供高效候选划分，再在原特征空间中评价划分质量。

---

## 3.3.4 递归锚点生成

对每个视图 $v$，初始化叶节点集合：

$$\mathcal{L}^{(v)}=\{X^{(v)}\}.$$

对于任意当前节点 $C\in\mathcal{L}^{(v)}$，若其样本数不满足最小划分条件：

$$n_C<2n_{\min},  
\tag{41}$$

则不再划分该节点，并将其作为叶节点。

若 $n_C\ge 2n_{\min}$，则按照式 $(32)$–$(38)$ 搜索最优候选划分，并计算最优得分：

$$S^\ast(C)=\max_k S_k.  
\tag{42}$$

若满足：

$$S^\ast(C)>\tau_{\mathrm{split}},  
\tag{43}$$

则将当前节点替换为两个子节点：

$$C\rightarrow C_1(k^\ast)\cup C_2(k^\ast).  
\tag{44}$$

否则，当前节点停止划分。

当所有叶节点均不再满足划分条件时，递归过程终止。最终，第 $v$ 个视图得到叶节点集合：

$$\mathcal{L}^{(v)}  
=  
\left\{  
C_1^{(v)},C_2^{(v)},\ldots,C_{m_v}^{(v)}  
\right\}.  
\tag{45}$$

每个叶节点对应一个锚点，其定义为该叶节点内所有样本的均值：

$$\theta_j^{(v)}  
=  
\frac{1}{|C_j^{(v)}|}  
\sum_{x_i^{(v)}\in C_j^{(v)}}  
x_i^{(v)},  
\qquad  
j=1,2,\ldots,m_v.  
\tag{46}$$

于是得到第 $v$ 个视图的锚点矩阵：

$$\Theta^{(v)}  
=  
\left[  
\theta_1^{(v)},\theta_2^{(v)},\ldots,\theta_{m_v}^{(v)}  
\right]^\top  
\in\mathbb{R}^{m_v\times d_v}.  
\tag{47}$$

该递归机制保留了 3AMVC 中“逐层二分并自动停止”的锚点生成思想，但将原 HBNC 的启发式停止依据替换为具有统计解释的 BIC 正则化似然比准则。3AMVC 原文指出，HBNC 的目标是在不预先指定簇数的情况下自动获得每个视图的高质量锚点。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

---

# 3.4 基于 BIC 证据校准的视图质量评价与基准视图选择

在完成第 $v$ 个视图的递归锚点选择后，得到叶节点集合：

$$\mathcal{L}^{(v)}
=
\left\{
C_1^{(v)},C_2^{(v)},\ldots,C_{m_v}^{(v)}
\right\}.$$

原始 3AMVC 使用叶节点簇内平方误差之和评价视图锚点质量，并选择该值最小的视图作为基准视图。该准则能够反映锚点划分后的紧凑性，但它仅刻画单视图内部误差，容易受到锚点数量、视图维度和特征尺度的影响。特别地，当某个视图生成更多锚点时，其簇内平方误差通常会下降，但这种下降并不必然说明该视图的锚点结构更可靠，也可能只是由模型复杂度增加带来的拟合能力提升。3AMVC 原文虽然根据簇内距离选择锚图质量最高的视图作为基准视图，但该评价本身并未显式校正锚点数量增加带来的复杂度。

本文的改进思想是：BIC-LR 锚点生成过程本身已经产生了与视图质量相关的统计证据，因此基准视图选择不应只依赖最终簇内误差，而应利用 BIC/LR 证据对锚点代表性进行校准。该校准仅用于选择对齐前的基准视图 $b$，不改变后续跨视图对齐目标，也不改变等权融合公式。

## 3.4.1 BIC-LR 阶段可复用的质量证据

对任意当前节点 $C$，BIC-LR 锚点选择比较如下两个局部模型：

$$H_0:\ C\ \text{由一个球形高斯分布生成},$$

$$H_1:\ C\ \text{由两个共享方差的球形高斯子分布生成}.$$

在单簇模型下，记节点样本数为 $n_C$、维度为 $d$、节点中心为 $\mu_0$，簇内平方误差为：

$$W_0
=
\sum_{x_i\in C}\|x_i-\mu_0\|_2^2.$$

带方差保护项的极大似然估计为：

$$\hat{\sigma}_0^2
=
\frac{W_0}{n_Cd}+\varepsilon_{\mathrm{var}},$$

单簇模型对数似然为：

$$\ell_0(C)
=
-\frac{n_Cd}{2}
\left[
\log(2\pi\hat{\sigma}_0^2)+1
\right].$$

对于候选划分 $C=C_1\cup C_2$，记 $n_1=|C_1|$、$n_2=|C_2|$、$\pi_1=n_1/n_C$、$\pi_2=n_2/n_C$，两个子节点的簇内平方误差分别为 $W_1$ 和 $W_2$。共享方差估计为：

$$\hat{\sigma}_{12}^2
=
\frac{W_1+W_2}{n_Cd}+\varepsilon_{\mathrm{var}},$$

双簇模型对数似然为：

$$\ell_1(C_1,C_2)
=
n_1\log \pi_1+n_2\log \pi_2
-
\frac{n_Cd}{2}
\left[
\log(2\pi\hat{\sigma}_{12}^2)+1
\right].$$

对应的 BIC 正则化似然比得分为：

$$S(C_1,C_2)
=
2\left[\ell_1(C_1,C_2)-\ell_0(C)\right]
-
\lambda_{\mathrm{BIC}}(d+1)\log n_C.$$

若当前节点的最优候选划分满足：

$$S^\ast(C)>\tau_{\mathrm{split}},$$

则接受分裂；否则停止分裂，并将当前节点中心作为锚点。本文主线中 $\tau_{\mathrm{split}}$ 默认固定为 $0$，不作为主实验搜索超参数。得分越大，表示该节点继续分裂的统计证据越强；BIC 惩罚项用于抑制单纯增加锚点数量导致的过拟合；最小节点规模 $n_{\min}$ 用于避免极小子簇和不稳定似然估计。

因此，每个视图在锚点生成结束后不仅得到锚点集合和锚点数，还自然得到叶节点簇内误差、接受分裂次数、候选分裂得分以及视图级证据增益等统计量。这些信息可作为基准视图选择阶段的质量证据。

## 3.4.2 锚点代表性误差与累计分裂证据

对第 $v$ 个视图，定义最终锚点划分的代表性误差为：

$$Q_v
=
\sum_{c=1}^{m_v}
\sum_{x_i^{(v)}\in C_c^{(v)}}
\left\|
x_i^{(v)}-\theta_c^{(v)}
\right\|_2^2.$$

该值越小，说明最终锚点对当前视图样本分布的代表性越强。与原始 3AMVC 的簇内误差质量准则一致，$Q_v$ 仍保留了“锚点应当紧凑表示样本”的基本含义。

另一方面，BIC-LR 的每次有效分裂均对应一个正的统计证据。记第 $v$ 个视图中被接受的分裂节点集合为 $\mathcal{T}_v^{+}$，第 $t$ 个被接受节点的 BIC-LR 得分为 $S_t$，则可定义累计分裂证据：

$$E_v
=
\sum_{t\in\mathcal{T}_v^{+}} S_t.$$

也可以定义平均分裂证据：

$$\bar{E}_v
=
\frac{E_v}{|\mathcal{T}_v^{+}|+\varepsilon}.$$

其中，$E_v$ 越大表示该视图中被接受的节点分裂具有越强的统计支持。与仅比较 $Q_v$ 不同，$E_v$ 显式反映了锚点生成过程中“为什么这些节点值得继续分裂”的证据强度。

## 3.4.3 比值型 BIC 证据校准准则

由于不同视图的特征维度、特征尺度、锚点数和累计证据范围可能不同，直接比较 $Q_v$ 与 $E_v$ 的原始数值并不稳妥。为提高跨视图可比性，可先进行视图间归一化：

$$\widetilde{Q}_v
=
\frac{Q_v}{\frac{1}{V}\sum_{j=1}^{V}Q_j+\varepsilon},$$

$$\widetilde{E}_v
=
\frac{E_v}{\frac{1}{V}\sum_{j=1}^{V}E_j+\varepsilon}.$$

在不希望新增超参数的设定下，优先采用比值型校准质量分数：

$$R_v
=
\frac{\widetilde{Q}_v}{\widetilde{E}_v+\varepsilon}.$$

最终选择：

$$b
=
\arg\min_v R_v.$$

该准则的含义是：$Q_v$ 越小越好，表示锚点代表性更强；$E_v$ 越大越好，表示被接受的分裂更有统计证据；因此 $R_v$ 越小，说明该视图同时具有较好的锚点代表性和较强的 BIC-LR 分裂证据。与减法型准则 $R_v=\widetilde{Q}_v-\eta\widetilde{E}_v$ 相比，比值型准则不需要额外引入 $\eta$，更符合当前主线“只搜索 $\beta/\lambda/\lambda_{\mathrm{BIC}}/n_{\min}$”的参数约束。

需要强调的是，上述准则只改变基准视图选择准则，不改变跨视图融合权重。BIC 证据校准的作用是提高“选择哪个视图作为对齐参考”的可靠性，而不是给不同视图分配融合权重。因此，该设计不引入 $\rho$、$\tau_S$ 或质量加权融合参数，仍然保持原 3AMVC 的等权融合形式。

从实现状态看，若系统尚未保存每个被接受分裂节点的 $S_t$，则上述 $E_v$ 和 $R_v$ 属于理论设计与待实现方案。实现该方案需要在递归分裂过程中记录所有接受分裂的得分，并在基准视图选择阶段读取这些得分计算 $E_v$。

## 3.4.4 当前实现中的整视图 BIC 证据增益形式

当前可直接落地的证据校准形式是将最终多锚点划分整体视为一个共享方差的多分量球形高斯模型，并与整视图单簇模型进行比较。该形式不要求保存每一次接受分裂的 $S_t$，只需要最终叶节点的样本数和簇内误差。

对第 $v$ 个视图，单簇整体模型的对数似然仍记为 $\ell_0^{(v)}$。最终多锚点模型的叶节点样本数为 $n_j^{(v)}$，混合比例为 $\pi_j^{(v)}=n_j^{(v)}/n$，总簇内误差为：

$$Q_v
=
\sum_{j=1}^{m_v}
\sum_{x_i^{(v)}\in C_j^{(v)}}
\left\|
x_i^{(v)}-\theta_j^{(v)}
\right\|_2^2.$$

共享方差估计为：

$$\hat{\sigma}_{T,v}^{2}
=
\frac{Q_v}{nd_v}+\varepsilon_{\mathrm{var}}.$$

最终多锚点模型的对数似然为：

$$\ell_T^{(v)}
=
\sum_{j=1}^{m_v}
n_j^{(v)}\log \pi_j^{(v)}
-
\frac{nd_v}{2}
\left[
\log(2\pi\hat{\sigma}_{T,v}^{2})+1
\right].$$

相对于单簇模型的整视图 BIC 证据增益定义为：

$$\Delta_{\mathrm{BIC}}^{(v)}
=
2\left(\ell_T^{(v)}-\ell_0^{(v)}\right)
-
\lambda_{\mathrm{BIC}}(m_v-1)(d_v+1)\log n.$$

其中，$(m_v-1)(d_v+1)\log n$ 对应从一个整体节点扩展到 $m_v$ 个叶节点所增加的复杂度惩罚。为进一步减弱不同视图样本规模和维度带来的量纲影响，可定义单位 BIC 证据增益：

$$U_{\mathrm{BIC}}^{(v)}
=
\frac{\Delta_{\mathrm{BIC}}^{(v)}}{\max(2nd_v,1)}.$$

当前实现采用 $U_{\mathrm{BIC}}^{(v)}$ 作为视图证据分数，并选择：

$$b
=
\arg\max_v U_{\mathrm{BIC}}^{(v)},
\qquad
q_v^{\mathrm{BIC}}=-U_{\mathrm{BIC}}^{(v)}.$$

整视图证据增益准则与前述比值型准则的共同点是：都利用 BIC/LR 证据校准单纯的簇内误差视图质量，并且都只作用于基准视图选择阶段。二者的差别在于：比值型准则依赖逐次接受分裂的累计证据 $E_v$，属于更细粒度的待实现设计；整视图证据增益使用最终多锚点模型相对于单簇模型的整体证据增益，属于当前代码中已经实现的证据校准口径。

该过程不会显著增加算法复杂度。最终多锚点模型所需的叶节点误差、叶节点样本数和锚点数可以在递归锚点生成过程中直接获得；单锚点整体模型所需的全局均值和根节点误差可以在处理每个视图的根节点时计算并保存。因此，若这些统计量已被保存，则 BIC 证据校准的额外复杂度为：

$$O\left(\sum_{v=1}^{V}m_v\right),$$

不会引入 $n\times n$ 样本图构造，也不会破坏锚图方法降低大规模多视图聚类复杂度的基本动机。

---

# 3.5 锚图学习

给定第 $v$ 个视图的锚点矩阵：

$$\Theta^{(v)}  
\in\mathbb{R}^{m_v\times d_v},$$

需要学习样本到锚点的关系矩阵：

$$Z^{(v)}  
\in\mathbb{R}^{n\times m_v}.$$

其中，第 $i$ 行 $z_i^{(v)}\in\mathbb{R}^{m_v}$ 表示样本 $x_i^{(v)}$ 与所有锚点之间的关联权重。为了使 $z_i^{(v)}$ 具有概率解释，要求：

$$z_i^{(v)}\ge 0,  
\qquad  
z_i^{(v)}\mathbf{1}_{m_v}=1.  
\tag{50}$$

因此，整个锚图矩阵满足：

$$Z^{(v)}\ge 0,  
\qquad  
Z^{(v)}\mathbf{1}_{m_v}=\mathbf{1}_{n}.  
\tag{51}$$

锚图学习可写为如下优化问题：

$$\min_{Z^{(v)},\Theta^{(v)}}  
\left\|  
X^{(v)}-Z^{(v)}\Theta^{(v)}  
\right\|_F^2  
+  
\beta  
\Omega(Z^{(v)}),  
\tag{52}$$ $$\text{s.t.}  
\quad  
Z^{(v)}\ge 0,  
\qquad  
Z^{(v)}\mathbf{1}_{m_v}=\mathbf{1}_{n}.  
\tag{53}$$

其中，第一项要求样本能够由锚点的加权组合进行重构，第二项 $\Omega(Z^{(v)})$ 是锚图正则项，$\beta>0$ 为正则化参数。3AMVC 论文中的多视图锚图学习目标同样采用样本—锚点重构形式，并要求锚图矩阵满足非负性与归一化约束。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

在该表示下，$Z_{ij}^{(v)}$ 表示第 $i$ 个样本与第 $j$ 个锚点之间的连接权重。由于：

$$\sum_{j=1}^{m_v}Z_{ij}^{(v)}=1,$$

因此每个样本在当前视图中被表示为所有锚点的凸组合。由此，锚图不仅描述样本与锚点之间的相似关系，也保留了样本在锚点空间中的结构分布。

---

# 3.6 跨视图锚图对齐

由于各视图独立执行自适应锚点选择，因此不同视图得到的锚点数量通常不同：

$$m_v\neq m_b.$$

于是，第 $v$ 个视图的锚图：

$$Z^{(v)}\in\mathbb{R}^{n\times m_v}$$

不能直接与基准视图锚图：

$$Z^{(b)}\in\mathbb{R}^{n\times m_b}$$

相加或融合。因此，需要学习一个将第 $v$ 个视图锚点空间映射到基准视图锚点空间的匹配矩阵：

$$P_v\in\{0,1\}^{m_v\times m_b}.$$

对齐后的第 $v$ 个视图锚图定义为：

$$\widetilde Z^{(v)}  
=  
Z^{(v)}P_v  
\in\mathbb{R}^{n\times m_b}.  
\tag{54}$$

对于基准视图，有：

$$\widetilde Z^{(b)}=Z^{(b)}.  
\tag{55}$$

为了同时考虑样本—锚点关系和锚点内部结构，定义第 $v$ 个视图的锚点结构图：

$$G^{(v)}  
=  
(Z^{(v)})^\top Z^{(v)}  
\in\mathbb{R}^{m_v\times m_v}.  
\tag{56}$$

基准视图的锚点结构图为：

$$G^{(b)}  
=  
(Z^{(b)})^\top Z^{(b)}  
\in\mathbb{R}^{m_b\times m_b}.  
\tag{57}$$

对齐后的第 $v$ 个视图锚点结构图为：

$$\widetilde G^{(v)}  
=  
P_v^\top G^{(v)}P_v  
\in\mathbb{R}^{m_b\times m_b}.  
\tag{58}$$

于是，非基准视图 $v\neq b$ 的对齐目标为：

$$\min_{P_v}  
\left\|  
Z^{(b)}-Z^{(v)}P_v  
\right\|_F^2  
+  
\lambda  
\left\|  
G^{(b)}-P_v^\top G^{(v)}P_v  
\right\|_F^2,  
\tag{59}$$ $$\text{s.t.}  
\quad  
P_v\in\mathcal{P}_{v\rightarrow b}\subseteq\{0,1\}^{m_v\times m_b}.
\tag{60}$$

其中，$\mathcal{P}_{v\rightarrow b}$ 表示从第 $v$ 个视图锚点到基准视图锚点的可行匹配集合，可表达一对一、多对一或一对多锚点关系。第一项对齐样本—锚点关系，第二项对齐锚点内部结构，$\lambda>0$ 用于控制结构一致性约束的重要性。3AMVC 论文中的跨视图对齐目标也由锚图关系项和锚点内部结构项组成，并指出匹配矩阵能够表达基准视图与其他视图之间的一对多或多对一锚点关系。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

该对齐过程的意义在于：不同视图的锚点虽然都来自同一批样本，但由于视图特征空间不同、锚点数量不同、局部划分结果不同，锚点索引之间并不天然对应。因此，需要通过 $P_v$ 将不同视图的锚点空间统一到基准视图锚点空间中。

BIC 证据校准与式 $(59)$ 的关系是前置而非替代：它只决定哪个视图作为 $b$，不改变对齐目标中的样本—锚点关系项、锚点结构项和对齐参数 $\lambda$。因此，该改进不是重新设计跨视图匹配矩阵，也不是改变最终融合方式，而是提高对齐参考视图选择的统计可靠性。

---

# 3.7 等权锚图融合

完成跨视图锚图对齐后，所有视图的锚图均被映射到基准视图的锚点空间中：

$$\widetilde Z^{(v)}  
\in\mathbb{R}^{n\times m_b},  
\qquad  
v=1,2,\ldots,V.$$

其中：

$$\widetilde Z^{(b)}=Z^{(b)}.$$

按照 3AMVC 论文中的融合策略，本文采用等权方式融合所有对齐后的锚图：

$$Z_{\mathrm{aligned}}  
=  
\frac{1}{V}  
\left(  
Z^{(b)}  
+  
\sum_{\substack{v=1\\v\neq b}}^{V}  
Z^{(v)}P_v  
\right).  
\tag{61}$$

也即：

$$Z_{\mathrm{aligned}}  
=  
\frac{1}{V}  
\sum_{v=1}^{V}  
\widetilde Z^{(v)}.  
\tag{62}$$

式 $(61)$ 与 3AMVC 论文中的对齐锚图融合公式一致。论文在获得各非基准视图的匹配矩阵后，将基准视图锚图与其他视图的对齐锚图相加并除以视图数，从而得到最终融合锚图。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

等权融合的含义是：在完成对齐后，每个视图都向最终共识锚图贡献相同权重。由于前一阶段已经将所有视图映射到基准视图锚点空间中，因此式 $(61)$ 可以直接在统一维度下融合不同视图的样本—锚点关系。与质量加权或对齐残差加权不同，等权融合不再额外引入视图权重参数，从而保持与 3AMVC 原论文一致的融合形式。

---

# 3.8 谱嵌入与最终聚类

得到融合锚图：

$$Z_{\mathrm{aligned}}  
\in\mathbb{R}^{n\times m_b}$$

后，对其进行奇异值分解：

$$Z_{\mathrm{aligned}}  
=  
U\Sigma V^\top.  
\tag{63}$$

其中：

$$U\in\mathbb{R}^{n\times n},  
\qquad  
\Sigma\in\mathbb{R}^{n\times m_b},  
\qquad  
V\in\mathbb{R}^{m_b\times m_b}.$$

取前 $K$ 个左奇异向量：

$$U_K=  
[u_1,u_2,\ldots,u_K]  
\in\mathbb{R}^{n\times K}.  
\tag{64}$$

最终，对 $U_K$ 的行向量执行 $K$-means 聚类：

$$\mathcal{Y}  
=  
K\text{-means}(U_K).  
\tag{65}$$

3AMVC 论文指出，在得到融合锚图后，对其进行谱分解，并在左奇异向量上执行 $K$-means，即可得到最终聚类结果。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

从图嵌入角度看，$U_K$ 给出了样本在融合锚图诱导的低维谱空间中的表示。若两个样本在多个视图中都具有相似的锚点连接结构，则它们在 $U_K$ 中的表示也会更接近，从而更容易被划分到同一簇中。

---

# 3.9 完整目标函数视角

整体上，本文方法可以被理解为三个层次的联合建模。

第一层是单视图自适应锚点生成：

$$\Theta^{(v)}  
=  
\mathcal{A}_{\mathrm{BIC-LR}}  
(X^{(v)}),  
\qquad  
v=1,2,\ldots,V.  
\tag{66}$$

其中，$\mathcal{A}_{\mathrm{BIC-LR}}$ 表示由式 $(1)$–$(47)$ 定义的递归锚点选择算子。该算子不预设锚点数 $m_v$，而是通过 BIC 正则化似然比得分自动决定节点是否继续分裂。

第二层是单视图锚图学习：

$$\min_{\{Z^{(v)},\Theta^{(v)}\}_{v=1}^{V}}  
\sum_{v=1}^{V}  
\left[  
\left\|  
X^{(v)}-Z^{(v)}\Theta^{(v)}  
\right\|_F^2  
+  
\beta\Omega(Z^{(v)})  
\right],  
\tag{67}$$ $$\text{s.t.}  
\quad  
Z^{(v)}\ge 0,  
\qquad  
Z^{(v)}\mathbf{1}_{m_v}=\mathbf{1}_{n},  
\qquad  
v=1,2,\ldots,V.  
\tag{68}$$

该目标与 3AMVC 中锚图学习的基本形式一致，即通过样本—锚点重构误差和锚图约束学习每个视图的锚图结构。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

第三层是跨视图对齐与等权融合：

$$\min_{\{P_v\}_{v\neq b}}  
\sum_{\substack{v=1\\v\neq b}}^{V}  
\left[  
\left\|  
Z^{(b)}-Z^{(v)}P_v  
\right\|_F^2  
+  
\lambda  
\left\|  
G^{(b)}-P_v^\top G^{(v)}P_v  
\right\|_F^2  
\right],  
\tag{69}$$ $$Z_{\mathrm{aligned}}  
=  
\frac{1}{V}  
\left(  
Z^{(b)}  
+  
\sum_{\substack{v=1\\v\neq b}}^{V}  
Z^{(v)}P_v  
\right).  
\tag{70}$$

因此，本文方法的整体逻辑可以概括为：

$$\text{BIC 正则化似然比检验决定锚点数}  
\Rightarrow  
\text{BIC 证据增益校准视图质量}  
\Rightarrow  
\text{证据增益最大的视图作为对齐基准}  
\Rightarrow  
\text{不同视图锚图对齐到统一锚点空间}  
\Rightarrow  
\text{等权融合得到共识锚图}  
\Rightarrow  
\text{谱嵌入获得最终聚类结果}.$$

---

# 3.10 算法描述

**输入：** 多视图数据 $\{X^{(v)}\}_{v=1}^{V}$，聚类数 $K$，锚图正则参数 $\beta$，对齐参数 $\lambda$，BIC 惩罚参数 $\lambda_{\mathrm{BIC}}$，最小节点规模 $n_{\min}$。其中，$\tau_{\mathrm{split}}$ 默认固定为 $0$，$\varepsilon_{\mathrm{var}}$ 仅作为数值稳定项。

**输出：** 聚类标签 $\mathcal{Y}$。

**步骤如下。**

1. 对每个视图 $v=1,\ldots,V$，初始化叶节点集合：

$$\mathcal{L}^{(v)}=\{X^{(v)}\}.$$

2. 对每个当前节点 $C$，计算节点中心 $\mu_C$ 与簇内平方误差 $W(C)$。
3. 构造局部分裂方向 $u$，将节点内样本投影到该方向上，并根据投影值排序。
4. 沿排序序列枚举所有满足最小节点规模约束的候选切分位置。
5. 对每个候选划分，分别计算单簇模型对数似然 $\ell_0(C)$ 和双簇模型对数似然 $\ell_1(C_1,C_2)$。
6. 根据式 $(30)$ 计算 BIC 正则化似然比得分。
7. 若最大得分 $S^\ast(C)$ 满足：

$$S^\ast(C)>\tau_{\mathrm{split}},$$

则接受该节点划分；否则将当前节点作为叶节点。

8. 当所有叶节点均不再继续划分时，令每个叶节点中心作为锚点，得到各视图锚点集合 $\Theta^{(v)}$。
9. 根据第 3.4 节计算每个视图的 BIC 证据校准质量分数。若保存了接受分裂得分，可使用 $R_v=\widetilde Q_v/(\widetilde E_v+\varepsilon)$ 并选择 $b=\arg\min_v R_v$；当前已实现口径使用单位 BIC 证据增益 $U_{\mathrm{BIC}}^{(v)}$ 并选择 $b=\arg\max_v U_{\mathrm{BIC}}^{(v)}$。
10. 根据式 $(52)$–$(53)$ 学习每个视图的锚图 $Z^{(v)}$。
11. 对每个非基准视图，根据式 $(59)$–$(60)$ 学习匹配矩阵 $P_v$，并得到对齐锚图 $Z^{(v)}P_v$。
12. 根据式 $(61)$ 或式 $(62)$ 对所有对齐锚图进行等权融合，得到 $Z_{\mathrm{aligned}}$。
13. 对 $Z_{\mathrm{aligned}}$ 进行奇异值分解，取前 $K$ 个左奇异向量 $U_K$。
14. 对 $U_K$ 的行向量执行 $K$-means，得到最终聚类标签 $\mathcal{Y}$。

---

# 3.11 复杂度分析

对于第 $v$ 个视图的某个节点 $C$，设其样本数为 $n_C$，特征维度为 $d_v$。构造局部分裂方向的计算代价近似为：

$$O(I n_C d_v),  
\tag{71}$$

其中 $I$ 表示局部二中心更新的迭代次数。

投影与排序的代价为：

$$O(n_Cd_v+n_C\log n_C).  
\tag{72}$$

若利用前缀统计量扫描候选切分点，则所有候选划分的簇内平方误差与似然得分可以在近似线性时间内计算，其复杂度为：

$$O(n_Cd_v).  
\tag{73}$$

因此，单个节点的划分评价复杂度约为：

$$O(I n_Cd_v+n_C\log n_C+n_Cd_v).  
\tag{74}$$

若第 $v$ 个视图最终生成 $m_v$ 个锚点，并且递归划分树近似平衡，则该视图锚点选择阶段的复杂度可近似表示为：

$$O(I n d_v\log m_v+n\log n).  
\tag{75}$$

与构造完整 $n\times n$ 样本相似图相比，该过程仍然避免了二次规模的图构造。3AMVC 原文也指出，其主要计算与样本数 $n$ 近似线性相关，能够更好适应大规模数据场景。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

锚图学习阶段主要涉及样本—锚点重构，复杂度与 $n$、$m_v$、$d_v$ 相关。跨视图对齐阶段主要依赖锚点数 $m_v$ 和基准视图锚点数 $m_b$，而不是完整样本数的平方。最终奇异值分解作用于 $n\times m_b$ 的融合锚图，其中 $m_b\ll n$，因此仍保持锚图方法在大规模聚类中的计算优势。

---

# 3.12 方法讨论

本文方法相对于原始 3AMVC 的主要变化在于：原 HBNC 使用基于邻域距离与层次二分的方式自动生成锚点，而本文将局部节点是否继续划分的问题改写为单簇模型与双簇模型之间的统计模型选择问题。3AMVC 原文指出，HBNC 的目标是在不指定簇数的情况下自动选择高质量锚点；本文保留该目标，但将停止准则替换为 BIC 正则化似然比判据。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview+1](https://openreview.net/pdf?id=TKRqWQVawP)

该替换带来三个主要优点。

第一，锚点数量的生成具有更清晰的统计解释。若当前节点的双簇模型在扣除 BIC 复杂度惩罚后仍优于单簇模型，则说明该节点内部存在值得保留的局部分裂结构，应继续生成更细粒度锚点；反之，则说明当前节点已足够紧凑，可以将其中心作为锚点。

第二，BIC 惩罚避免了单纯最大化似然导致的过划分问题。由于双簇模型参数量更多，若不加入复杂度惩罚，模型会倾向于不断分裂节点。式 $(30)$ 中的 $(d+1)\log n_C$ 项正是对新增参数复杂度的惩罚。

第三，该改进仍然保持 3AMVC 的视图自适应性质。不同视图独立执行递归似然比划分，因此可以根据自身数据结构生成不同数量的锚点。随后，跨视图对齐模块将不同数量的锚点映射到基准视图锚点空间，最后按照 3AMVC 原论文中的等权融合公式得到最终共识锚图。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview](https://openreview.net/pdf?id=TKRqWQVawP)

---

# 3.13 代码实现对应关系

当前仓库中，原始 3AMVC 与 BIC-LR 改进路径并行保留。原始演示入口位于 `3AMVC-main/demo.m`，其调用链为 `Neighbor.m -> Pre_HBNC.m / Impro_HBNC.m -> algo_qp.m -> aligned.m -> DSPFP.m`。其中，原始基准视图选择通过比较各视图锚点簇内误差之和完成，即选择 `object_sum` 最小的视图作为 `target_view`。

BIC-LR 改进入口包括 `3AMVC-main/demo_biclr.m` 和 `3AMVC-main/run_biclr_grid_search.m`。单视图锚点生成由 `Neighbor_BICLR.m` 调用 `biclr/BIC_LR_HBNC.m` 完成；核心节点切分、似然和误差计算分别对应 `biclr_best_split.m`、`biclr_loglik_single.m`、`biclr_loglik_double.m` 和 `biclr_node_sse.m`。这些文件实现了单簇模型、双簇模型、BIC 正则化似然比得分以及 `S_best > tauSplit` 的分裂接受准则。

当前代码已经实现的基准视图证据校准位于 `biclr/biclr_view_evidence.m` 和 `biclr/biclr_select_target_view.m`。前者根据最终锚点划分计算 `deltaBIC`、`unitGain` 和 `qualityScore`；后者读取每个视图的 `unitGain`，选择单位 BIC 证据增益最大的视图作为 `targetView`，并将 `qualityScores` 记录为 `-unitGains` 以兼容“质量分越小越好”的旧接口。`run_biclr_grid_search.m` 中的缓存准备逻辑会保存 `thetaAll`、`objectAll`、`labelAll`、`infoAll`、`qualityScores` 和 `targetView` 等信息，并在缓存键中包含数据集、预处理、视图编号、`lambdaBIC`、`minNodeSize`、`tauSplit`、`epsVar` 和随机种子。

跨视图对齐和融合仍由 `algo_qp.m`、`aligned.m` 和 `DSPFP.m` 负责。`algo_qp.m` 在主优化收敛后调用 `aligned(Zi, lambda, target_view)`；`aligned.m` 以基准视图锚图为参考，调用 `DSPFP.m` 求解锚点匹配关系，并在所有视图对齐后执行 `S = S / numview` 的等权融合。当前 `aligned.m` 中存在基于锚图关系相似度的对齐矩阵构造与列归一化处理，它作用于非基准视图到基准锚点空间的映射，不是跨视图质量加权融合，也不引入 `rho`、`tauS`、`quality_weighted` 或 `quality_alignment_weighted` 作为当前主方法参数。

需要区分的是：第 3.4.4 节的整视图 BIC 证据增益已经写入代码；第 3.4.2–3.4.3 节中基于接受分裂集合 $\mathcal{T}_v^{+}$ 的累计证据 $E_v=\sum_t S_t$ 与比值型 $R_v=\widetilde Q_v/(\widetilde E_v+\varepsilon)$ 仍属于理论设计与待实现方案。若后续要实现该更细粒度版本，需要在 `BIC_LR_HBNC.m` 接受分裂时保存每个 `splitInfo.score`，在 `info` 中增加类似 `acceptedSplitScores` 或 `splitEvidence` 的字段，并在 `biclr_select_target_view.m` 中加入基于 $Q_v/E_v$ 的选择分支。

---

# 3.14 与消融实验的关系

该改进可以通过以下对照验证其作用边界。

1. **Original 3AMVC**：使用原始 HBNC 锚点选择、原始簇内误差基准视图选择、原始跨视图对齐框架和等权融合。该组用于衡量 BIC-LR-3AMVC 相对原论文路径的整体变化。

2. **BIC-LR-3AMVC**：使用 BIC-LR 锚点选择、BIC 证据校准基准视图选择、原 3AMVC 对齐框架和等权融合。当前代码中的主线口径为整视图单位 BIC 证据增益；若后续实现累计分裂证据版本，则可替换为第 3.4.3 节的比值型基准视图选择准则。

3. **w/o BIC**：去掉 BIC 惩罚项，使分裂准则退化为未惩罚的似然比增益。该组用于观察锚点数、接受分裂次数和聚类指标是否出现过划分相关变化。

4. **w/o LR**：去掉似然比建模，使用原始 HBNC 或等价启发式锚点生成路径。该组用于验证 BIC-LR 局部统计建模相对原始启发式切分的贡献。

5. **w/o alignment**：保留 BIC-LR 锚点选择，但去掉跨视图锚点对齐，以验证对齐步骤对多视图信息融合的必要性。由于不同视图锚点数通常不一致，直接对未对齐锚图做等权平均在维度上不可定义；当前仓库的消融文档中更可执行的代理形式是 `A4_woMultiViewFusion`，即仅使用目标视图的单视图锚图聚类。若后续需要严格的 w/o alignment 变体，应先明确统一锚点维度或未对齐融合的合法定义。

上述消融均围绕锚点选择、BIC 惩罚、似然比建模、基准视图选择和对齐融合的主线因素展开。除非结果文件中确有对应实现和记录，不应额外加入 `w/o projection scanning` 或 `w/o minNodeSize` 等低边际信息增益的消融组。

---

# 参考文献对应关系

本文方法各部分主要对应如下文献来源。

1. **3AMVC 总体框架、HBNC 自动锚点选择、原始簇内误差视图质量评价、基准视图选择、跨视图锚图对齐、等权融合与谱聚类**：Automatic and Aligned Anchor Learning Strategy for Multi-View Clustering，ACM MM 2024。[![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview+2![](https://www.google.com/s2/favicons?domain=https://openreview.net&sz=128)OpenReview+2](https://openreview.net/pdf?id=TKRqWQVawP)
2. **BIC 模型选择准则**：Schwarz, G. “Estimating the Dimension of a Model,” Annals of Statistics, 1978。本文式 $(30)$ 中的复杂度惩罚项来自 BIC 模型选择思想。[![](https://www.google.com/s2/favicons?domain=https://sites.stat.washington.edu&sz=128)华盛顿大学统计网站](https://sites.stat.washington.edu/courses/stat527/s13/readings/ann_stat1978.pdf?utm_source=chatgpt.com)
3. **基于 BIC 的自动簇数估计思想**：X-means 通过 BIC 判断局部簇是否继续分裂，从而避免预先固定簇数；本文的局部节点分裂判定与其模型选择思想一致。[![](https://www.google.com/s2/favicons?domain=https://www.cs.cmu.edu&sz=128)CMU School of Computer Science+1](https://www.cs.cmu.edu/~dpelleg/download/xmeans.pdf?utm_source=chatgpt.com)
4. **似然比检验在非标准条件下的注意事项**：Self and Liang 对非标准条件下最大似然估计与似然比统计量的渐近性质进行了讨论，因此本文采用 BIC 正则化似然比作为局部划分准则，而不是直接使用固定卡方阈值。[![](https://www.google.com/s2/favicons?domain=https://www.stat.cmu.edu&sz=128)卡内基梅隆大学统计与数据科学](https://www.stat.cmu.edu/~brian/763-2015/week06/papers/self-liang-1987.pdf?utm_source=chatgpt.com)
